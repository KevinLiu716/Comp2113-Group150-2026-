...
### Save / load (File I/O) — quit anytime!
The game auto-saves to `shelter_save.txt` at the start of every day, and
again at the end of every completed day.

**You can type `q` (or `Q`) at any input prompt to save and quit
immediately.** A confirmation screen tells you which day you will resume
on. The next time you launch the game, a "Save File Found" prompt asks
whether to continue.

Saves include every survivor's name, status, sick day count, trait, plus
all resources, items, flags, and the current day. The save is written
in a simple line-based text format that is easy to inspect and debug.

Note: when you quit mid-day with `q`, the day's progress so far is
discarded — you resume from the start of that day. This keeps the save
state simple and predictable.
